...
def _do_cmdline(args):
    ...
    if len(args) != 0:
        raise Failure, "Unexpected non-option arguments given"
    ...
    for r in rules:
        ...
        if isinstance(r.obj.vcs, muddled.vcs.git.Git):
            ...
            for h in raw_heads:
                m = pat.match(h)
                if m is None:
                    raise Failure("Unparseable output from git: %s"%h)
                heads.add(m.group(1))
            ...
            if g.branch is not None:
                if g.branch in heads:
                    ...
                    if bfrom in heads:
                        ...
                        raise Failure("Error: %s wants a branch named '%s', does not have one, and does not have a '%s' either - I don't know how to fix this"%(co_dir, g.branch, bfrom))
            else:
                # want master, don't care about others
                if verbose: 
                    print "%s heads are: %s"%(co_dir,heads)
                if not 'master' in heads:
                    raise Failure("Error: %s wants a 'master' branch but does not have one, I don't know how to fix this"%co_dir)
        else:
            if verbose:
